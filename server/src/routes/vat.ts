...
import { getVatSummary } from "../services/vatService.js";

const router = Router();
// Mali veri: operator/viewer goremez - expenses.ts/supplierLedger.ts/cashAccounts.ts/
// profitLoss.ts ile ayni gerekce, on muhasebe serisinin tutarliligi icin.
...
router.get("/", validateQuery(querySchema), (req, res) => {
  ...
  res.json({ summary: getVatSummary(req.stationId!, q.from, q.to) });
});
